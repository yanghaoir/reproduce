...
#
# 用法：
#   bash sh/eval.sh <MODEL> <GPU_IDS> [DATASETS]
#
# 参数：
#   MODEL     - 模型名称 (必须)
...
#
# 示例：
#   bash sh/eval.sh model1 0,1,2,3                        # 4卡评估所有数据集
#   bash sh/eval.sh model1 1,2,3,5,7 docvqa,slidevqa      # 5卡评估指定数据集
#   bash sh/eval.sh model1 1,2,3,5,7 [docvqa,slidevqa]    # 同上，方括号可选
#   bash sh/eval.sh model1 3                               # 单卡评估所有数据集
#
# 注意：本项目使用 conda 环境 vdoc（与 VDocRAG 的 vdocrag 不同）
# ------------------------------------------------------------------
set -e
