...
from vdocrag.vdocretriever.arguments import ModelArguments, VDocRetrieverTrainingArguments as TrainingArguments, DataArguments
...
class VDocRetriever(nn.Module):
    ...
    def forward(self, query: Dict[str, Tensor] = None, 
        ...
        # 编码describe文本（完整batch，大小为B，text样本使用占位符编码）
        # 所有GPU都执行此操作，确保ZeRO-3参数收集同步
        ...
        if self.training:
            # 汇聚所有GPU的表示
            # 由于每个GPU的batch大小B相同，all_gather张量大小一致，不会出错
            if self.is_ddp:
            ...
            # === 全batch对比学习损失 ===
            # 在完整batch上计算对比学习，最大化有效batch size
            ...
            if self.is_ddp:
                loss = loss * self.world_size  # 补偿DDP梯度平均
    ...
    def compute_kl_loss(self, teacher_scores, student_scores):
        ...
        计算KL散度loss
        Args:
            teacher_scores: query_text对document的相似度分数 (已除以temperature)
            student_scores: describe对document的相似度分数 (已除以temperature)
        Returns:
            KL散度loss
        """
    ...
    def gradient_checkpointing_enable(self, **kwargs):
        if self.is_qwen:
            ...
            elif hasattr(self.encoder, 'model') and hasattr(self.encoder.model, 'gradient_checkpointing_enable'):
                self.encoder.model.gradient_checkpointing_enable(**kwargs)
            else:
                logger.warning("gradient_checkpointing_enable not found, skipping")
        else:
    ...
    def encode_query(self, qry, use_cache=True):
        if self.is_qwen:
            outputs = self.encoder(**qry, return_dict=True, output_hidden_states=True, use_cache=use_cache)
            if hasattr(outputs, 'hidden_states') and outputs.hidden_states is not None:
                query_hidden_states = outputs.hidden_states[-1]
            elif hasattr(outputs, 'last_hidden_state'):
                query_hidden_states = outputs.last_hidden_state
            else:
                query_hidden_states = outputs[0]
        else:
            query_hidden_states = self.encoder(**qry, return_dict=True, output_hidden_states=True, output_attentions=True, use_cache=use_cache)
