...
class DataArguments:
    ...
    # 此处为修改的代码
    # 添加训练模式参数，支持四种训练方式：只使用text_batch、只使用image_batch、先text后image、混合训练
    ...
    # 此处为修改的代码
    # 添加KL散度loss的权重参数，用于控制知识蒸馏的强度
    kl_loss_weight: float = field(
        default=1.0,
        metadata={"help": "Weight for KL divergence loss in image batch training"}
    )

    # 此处为修改的代码
    # 添加query_image和relevant_doc_ids的采样策略参数
