...
class TrainCollator:
    ...
    def _encode_images(self, images):
        """根据模型类型编码图像，返回 (input_ids_list, image_meta)"""
        if self.is_qwen:
            messages_list = [[{
                "role": "user",
                "content": [
    ...
    def _attach_image_tensors(self, target_dict, image_meta):
        """将图像张量（pixel_values 等）附加到 collated dict"""
        if self.is_qwen:
    ...
    def __call__(self, features: List[Tuple[str, List[str]]]):
        ...
        if len(features[0]) == 2:
            ...
            # 新格式: (query_text, query_image, relevant_doc_image, query_type, describe)
            all_queries = [f[0] for f in features]  # query_text
            all_images = [f[2] for f in features]  # relevant_doc_image
            query_types = [f[3] for f in features]  # query_type
            describes = [f[4] if len(f) > 4 else '' for f in features]

        # 创建 query_type_ids 张量 (0=text, 1=image)
        # 所有GPU上batch大小一致(B)，因此此张量大小一致，可安全all_gather
        query_type_ids = torch.tensor(
            [1 if qt == 'image' else 0 for qt in query_types],
            dtype=torch.long
        )

        # 编码query文本（所有样本都需要编码query_text）
        ...
        # 为所有样本编码describe文本
        # text类型样本使用占位符"N/A"，其编码结果不会参与loss计算
        # 但必须为所有样本编码，确保ZeRO-3下所有GPU的encoder调用次数一致
        all_describes_text = []
        for i in range(len(features)):
            if query_types[i] == 'image' and describes[i] and describes[i].strip():
                all_describes_text.append(describes[i])
            else:
                all_describes_text.append("N/A")  # 占位符
        ...
        # 返回格式: (q_collated, d_collated, p_collated, q_describe_collated, query_type_ids)
        # 所有张量的batch维度大小均为B，在所有GPU上一致
